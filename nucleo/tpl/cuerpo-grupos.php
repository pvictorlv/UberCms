...
$bg = "";
$my_membership = dbquery("SELECT * FROM groups_memberships WHERE groupid = '".(filter($_GET['id']))."' AND userid ='".USER_ID."'");
$member_rank = $my_membership['member_rank'];

if(isset($_GET['id']) && is_numeric($_GET['id']))
{
	$check = dbquery("SELECT * FROM groups_details WHERE id = '".(filter($_GET['id']))."' LIMIT 1");
	$exists = $check->num_rows;
...
?>
<title><?php include ("nombre.php"); ?>: <?php echo ($groupdata['name']); ?></title>
<?php 
...
<div class="box-tabs-container box-tabs-left clearfix">
	<?php if($member_rank > 1 && LOGGED_IN && $edit_mode == false){ ?><a href="%www%/editar_grupos?inicio" id="myhabbo-group-tools-button" class="new-button dark-button edit-icon" style="float:left"><b><span></span>Editar</b><i></i></a><?php } ?>
