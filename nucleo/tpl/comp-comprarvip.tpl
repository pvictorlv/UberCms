...
db::query("UPDATE users SET vip = '1' WHERE id=".$id."");
...
echo 'Felicidades '.USER_NAME.', Compra realizada con �xito';
}else{$error = "No tienes suficientes coins.";echo $error;};
}else{echo '<form method="POST" name="buyvip"><select name="mesvip"><option selected="selected" value="1">1 Mes 30 Coins</option><option value="3">3 Meses 50 Coins</option><option value="6">6 Meses 70 Coins</option><option value="12">1 A�o 90 Coins</option></select><input name="enviar" class="buttonv4" value="Comprar" type="submit"></form>';};
