<?php
$query = dbquery("SELECT * FROM users WHERE id = '" . USER_ID . "'");
...
while ($data = mysqli_fetch_array($query)) {
    ...
    $saldo = $data['VIP_Coins'];
}

if (isset($_POST['buy-bot'])) {
    $user_info = @dbquery("SELECT * FROM users WHERE username='" . clean($_POST["habbo"]) . "'");
    if ($row = mysqli_fetch_assoc($user_info)) {
        $id = $row['id'];
        $owner = $row['id'];
        $pixeles = $row['activity_points'];
        $creditos2 = $row['VIP_Coins'];
        ...
        $check_bot = dbquery("SELECT * FROM bots WHERE owner='" . $id . "'");
        $check_user = dbquery("SELECT * FROM users WHERE id='" . $id . "'");
        $check_stats = dbquery("SELECT * FROM user_stats WHERE id='" . $id . "'");

        if (mysqli_num_rows($check_bot) > 20) {
            $error .= "�Por Ahora Solo se Permite 20 BOT por Usuario!<br /><br />";
        } else {
            if ($mostrar = mysqli_fetch_assoc($check_stats)) {
                $respetos = $mostrar['Respect'];
                if ($saldo >= $precio) {
                    $creditos_totales = $saldo - $precio;


                    dbquery("UPDATE users SET VIP_Coins = '$creditos_totales' WHERE id = '$id'");
                    dbquery("INSERT INTO `bots`(`room_id`, `name`, `motto`, `look`, `x`, `y`, `rotation`, `walk_mode`, `owner`) VALUES ('$sala', '$nombre', '$mision', '$ropa', '$posicionx', '$posiciony', '$rotacion', '$estado', '$owner')");


                    $error .= "<font color='#009900'>Felicidades!! Acabas de recibir  un BOT Ahora Tienes " . $creditos_totales . " Vip Coins<br> PARA VER EL BOT DEBES ESCRIBIR :update_bots  Y LUEGO :unload PARA RECARGAR LA SALA.<br /></font>";
                } else {
                    $error .= "<font color='#FF0000'>No tienes suficientes Vip Coins para realizar la compra!!<br /><br /></font>";
                }
            }
        }
    }
...
<?php $query = dbquery("SELECT * FROM rooms WHERE owner ='" . USER_NAME . "'");
...
Selecciona un Look Para tu BOT
    <?php
    $query = dbquery("SELECT * FROM users WHERE id = '" . USER_ID . "'") or die(mysqli_error());
...
<input name="habbo" type="hidden" id="habbo" value="%habboName%"/>
    ...
    $query = dbquery("SELECT * FROM users WHERE username = '" . $me . "'");
    while ($data = mysqli_fetch_array($query)) {
        $rango = $data['rank'];
        $vip = $data['vip'];
    }
