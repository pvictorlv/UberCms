...
<form action="#" method="post" id="group-settings-form">
  ...
  <div id="group-settings">
    <div id="group-settings-data" class="group-settings-pane">
      <div id="group-logo">
        <img src="/habbo-imaging/badge/<?php echo $row['badge']; ?>.gif" />
      </div>
      <div id="group-identity-area">
        <div id="group-name-area">
          ...
          <input type="text" name="group_name" id="group_name" onKeyUp="GroupUtils.validateGroupElements('group_name', 30, 'Has alcanzado el l�mite');" value="<?php echo CleanText($row['name']); ?>"/><br />
        </div>

        <div id="group-url-area">
          <div id="group_url_message_error" class="error"></div>
			<?php if(empty($row['nameUrl'])) { ?>
            <label for="group_url" id="group_url_text">Edit Group URL:</label><br/>
            <input type="text" name="group_url" id="group_url" onKeyUp="GroupUtils.validateGroupElements('group_url', 30, 'URL limit reached');" value="<?php echo CleanText($row['nameUrl']); ?>"/><br />
            ...
            <span id="group_url_text"><a href="<?php echo SITE; ?>/groups/<?php echo $row['nameUrl']; ?>">..groups/<?php echo $row['nameUrl']; ?></a></span><br/>
            <input type="hidden" name="group_url" id="group_url" value="<?php echo $row['nameUrl']; ?>"/>
        ...
        <div id="group-description-area">
          ...
          <textarea name="group_description" id="group_description" onKeyUp="GroupUtils.validateGroupElements('group_description', 255, 'La descripci�n del Grupo es demasiado larga');"><?php echo CleanText($row['description']); ?></textarea>
        </div>
      </div>
...
<?php 
$sql = Db::query("SELECT id,caption,description FROM rooms WHERE owner = '".$name."' ");
$groupdetails = Db::query("SELECT * FROM groups_details WHERE id = '".$groupid."' LIMIT 1");
...
<div id="room-settings-id" class="group-settings-pane-wide group-settings-selection">
	...
	<?php while($row = $sql->fetch(PDO::FETCH_ASSOC)) {
		        $i++;
		?>
    	<li class="<?php if($System->isEven($i) == "even") { echo "even"; } else { echo "odd"; } ?>">
    		<input type="radio" name="roomId" value="<?php echo $row['id']; ?>" <?php if($group['roomid'] == $row['id']) { echo "checked=\"checked\""; } ?> /><a href="/client?forwardId=2&roomId=<?php echo $row['id']; ?>" onclick="HabboClient.roomForward(this, '<?php echo $row['id']; ?>', 'private'); return false;" target="<?php echo $coins['client_token']; ?>" class="room-enter">Entrada</a><div>
