 ...
 isset($_POST['message']) && isset($_POST['recaptcha_challenge_field']) && isset($_POST['recaptcha_response_field']) && isset($_POST['page'])) {
	...
	if($GroupData->rowCount()) {
		...
		if($TopicData->rowCount()) {
			...
			$GroupData = $GroupData->fetch(PDO::FETCH_ASSOC);
			$forumType = $TopicData->fetch(PDO::FETCH_ASSOC);
...
<div class="postlist-header clearfix">
							...
							<?php }
	$count = ->rowCount(db::query("SELECT null FROM groups_forum_replies WHERE topic_id = ?';"));
	$n = $count;
	$x = 0;
		while($n >= 0)
		{
