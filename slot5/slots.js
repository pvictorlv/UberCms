...
var starting_credits = u_credits;
...
var reel_area_left = 60;
var reel_area_top = 34;
var reel_area_width = 96;
var reel_area_height = 96;

// set up reels
var reels = new Array(reel_count);
reels[0] = [2, 1, 7, 1, 2, 7, 6, 7, 3, 10, 1, 6, 1, 7, 3, 4, 3, 2, 4, 5, 0, 6, 10, 5, 6, 5, 8, 3, 0, 9, 5, 4];
reels[1] = [6, 0, 10, 3, 6, 7, 9, 2, 5, 2, 3, 1, 5, 2, 1, 10, 4, 5, 8, 4, 7, 6, 0, 1, 7, 6, 3, 1, 5, 9, 7, 4];
reels[2] = [1, 4, 2, 7, 5, 6, 4, 10, 7, 5, 2, 0, 6, 4, 10, 1, 7, 6, 3, 0, 5, 7, 2, 3, 9, 3, 5, 6, 1, 8, 1, 3];
...
function draw_symbol(symbol_index, x, y) {
    ...
    ctx.drawImage(symbols, 0, symbol_pixel, symbol_size, symbol_size, x + reel_area_left, y + reel_area_top + 10, symbol_size, symbol_size);
}
...
function render_reel() {
    ...
    // set clipping area
    ctx.beginPath();
    ctx.rect(reel_area_left, reel_area_top + 10, reel_area_width, reel_area_height);
    ctx.clip();
...
function highlight_line(line_num) {
    ...
    if (line_num == 2 || line_num == 4) {
        ctx.strokeRect(reel_area_left, reel_area_top + 10, symbol_size - 1, symbol_size - 1); // top left
    }
    if (line_num == 2) {
        ctx.strokeRect(reel_area_left + ss, reel_area_top + 10, ss - 1, ss - 1); // top middle
    }
    if (line_num == 2 || line_num == 5) {
        ctx.strokeRect(reel_area_left + ss + ss, reel_area_top + 10, ss - 1, ss - 1); // top right
    }

    // middle row
    if (line_num == 1) {
        ctx.strokeRect(reel_area_left, reel_area_top + 10 + ss, ss - 1, ss - 1); // top left
    }
    if (line_num == 1 || line_num == 4 || line_num == 5) {
        ctx.strokeRect(reel_area_left + ss, reel_area_top + 10 + ss, ss - 1, ss - 1); // top middle
    }
    if (line_num == 1) {
        ctx.strokeRect(reel_area_left + ss + ss, reel_area_top + 10 + ss, ss - 1, ss - 1); // top right
    }

    // bottom row
    if (line_num == 3 || line_num == 5) {
        ctx.strokeRect(reel_area_left, reel_area_top + 10 + ss + ss, ss - 1, ss - 1); // top left
    }
    if (line_num == 3) {
        ctx.strokeRect(reel_area_left + ss, reel_area_top + 10 + ss + ss, ss - 1, ss - 1); // top middle
    }
    if (line_num == 3 || line_num == 4) {
        ctx.strokeRect(reel_area_left + ss + ss, reel_area_top + 10 + ss + ss, ss - 1, ss - 1); // top right
    }
...
function logic_reward() {
    ...
    payout--;
    credits++;
    cred_p.innerHTML = "Seus cr&eacute;ditos: " + credits + "";
...
function calc_reward() {
    ...
    if (partial_payout > 0) {
        log_p.innerHTML += "Linha 1 pagou " + partial_payout + "&nbsp; ";
    ...
    if (playing_lines > 1) {
        ...
        if (partial_payout > 0) {
            log_p.innerHTML += "Linha 2 pagou " + partial_payout + "&nbsp; ";
            payout += partial_payout;
            highlight_line(2);
        }
        ...
        if (partial_payout > 0) {
            log_p.innerHTML += "Linha 3 pagou " + partial_payout + "&nbsp; ";
            payout += partial_payout;
            highlight_line(3);
        }
    }
    ...
    if (playing_lines > 3) {
        ...
        if (partial_payout > 0) {
            log_p.innerHTML += "Linha 4 pagou " + partial_payout + "&nbsp; ";
            payout += partial_payout;
            highlight_line(4);
        }
        ...
        if (partial_payout > 0) {
            log_p.innerHTML += "Linha 5 pagou " + partial_payout + "&nbsp; ";
            payout += partial_payout;
            highlight_line(5);
        }
    }
