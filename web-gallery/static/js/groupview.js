...
function initView(C, D) {
    var K = $("leave-group-button");
    if (K) {
        Event.observe(K, "click", function (N) {
            Event.stop(N);
            openGroupActionDialog("habblet/ajax_groups_confirm_leave.php", "habblet/ajax_leavegroup.php", null, C, null)
        })
    }
    var G = $("join-group-button");
    if (G) {
        Event.observe(G, "click", function (N) {
            Event.stop(N);
            openGroupJoinDialog("habblet/ajax_joingroup.php", C)
        })
    }
    ...
    var E = $("inventory-button");
    if (E) {
        Event.observe(E, "click", function (N) {
            Event.stop(N);
            loadWebStore(function () {
                if (window.WebStore) {
    ...
    var H = $("reporting-button");
    if (H) {
        Element.show(H);
        Event.observe(H, "click", startReportingModeObserver, false);
        Event.observe("stop-reporting-button", "click", stopReportingModeObserver, false)
    }
    var M = $("myhabbo-group-tools-button");
    if (M) {
        GroupEditTools.init(C, M)
    }
    var I = $("group-memberlist");
    if (I) {
        MembersList.init(C, D)
    }
    var J = $("select-favorite-button");
    var F = $("deselect-favorite-button");
    if (J || F) {
        GroupFavoriteSelector.init(null, D, C, function () {
        ...
        if (F) {
            Event.observe(F, "click", GroupFavoriteSelector.deselectFavorite)
        }
    }
    if (!!location.hash) {
...
function openGroupActionDialog(J, H, F, D, E, C) {
    ...
    new Ajax.Request(habboReqPath + J, {
        method: "post", parameters: I, onComplete: function (L, K) {
...
function openGroupJoinDialog(A, D) {
    ...
    new Ajax.Request(habboReqPath + A, {
        method: "post", parameters: {groupId: D}, onComplete: function (G, F) {
...
function showGroupSettingsConfirmation(D) {
    ...
    var G = $F("group_url");
    var H = $F("group_url_edited");
    var F = {url: G, groupId: D};
    if (G == "" || H == 0) {
        confirmAndUpdateGroupSettings(D)
    } else {
        new Ajax.Request(habboReqPath + "check_group_url.php", {
            method: "post", parameters: F, onComplete: function (N, M) {
...
function updateGroupSettings(A) {
    new Ajax.Request(habboReqPath + "habblet/update_group_settings.php", {
        parameters: {
            name: $F("group_name"),
...
function openGroupSettings(C) {
    ...
    A.show();
    A.style.zIndex = "9001";
    new Ajax.Updater("dialog-group-settings-body", habboReqPath + "habblet/ajax_group_settings.php", {
        parameters: {groupId: C},
        evalScripts: true,
        method: "post"
    });
    Overlay.show();
...
var MembersList = {
    init: function (A, C) {
        ...
        if (!MembersList.loadingMembers) {
            ...
            new Ajax.Updater("group-memberlist-members", habboReqPath + "habblet/ajax_groups_memberlist.php", {
                method: "post",
                parameters: {pageNumber: D, groupId: MembersList.groupId, searchString: A},
        ...
        if (!MembersList.loadingPending) {
            ...
            new Ajax.Updater("group-memberlist-pending", habboReqPath + "habblet/ajax_groups_memberlist.php", {
                method: "post",
        ...
        if (D.innerHTML == "") {
            ...
            new Ajax.Request(habboReqPath + "groups_memberlist_avatarinfo.php", {
                method: "post",
        ...
        if (E.length > 0) {
            ...
            new Ajax.Request(habboReqPath + "habblet/ajax_groups_confirm_" + C + ".php", {
                method: "post", parameters: {groupId: MembersList.groupId, targetIds: E}, onComplete: function (G, F) {
                    ...
                    if ($("group-action-ok")) {
                        Event.observe($("group-action-ok"), "click", function (H) {
                            Event.stop(H);
                            Dialog.setAsWaitDialog(D);
                            new Ajax.Request(habboReqPath + "habblet/ajax_groups_" + C + ".php", {
                                method: "post",
                                parameters: {groupId: MembersList.groupId, targetIds: E},
...
function openGroupBadgeEditor(C) {
    ...
    A.show();
    A.style.zIndex = "9001";
    new Ajax.Updater("badge-editor-dialog-body", habboReqPath + "habblet/ajax_show_badge_editor.php", {
        parameters: {groupId: C}, evalScripts: true, method: "post", onComplete: function (E, D) {
...
LinkTool.prototype = {
    initialize: function (A) {
        ...
        Event.observe("linktool-query", "keypress", function (C) {
        ...
        if (!this.textarea.getSelection() || this.textarea.getSelection() == "") {
            this.textarea.replaceSelection("[" + A + "=" + C + "]" + D + "[/" + A + "]")
        } else {
            this.textarea.wrapSelection("[" + A + "=" + C + "]", "[/" + A + "]")
        }
    }, search: function (D) {
        Event.stop(D);
        $("linktool-results").style.display = "block";
        Element.wait($("linktool-results"));
        var C = $F("linktool-query");
        var A = 1;
        $$(".linktool-scope").each(function (E) {
            if (E.checked) {
                A = E.value
            }
        });
        new Ajax.Updater("linktool-results", habboReqPath + "habblet/search.php", {
            method: "get",
            parameters: {query: C, scope: A}
        })
    }
};
...
var Discussions = {
    ...
    initialize: function (D, A, C) {
        if (!Discussions.initialized) {
            ...
            if (Discussions.groupdId) {
                ...
                Discussions.redirectLocation = habboReqPath + "forum.php"
            }
    ...
    createTopic: function () {
        var A = Discussions.baseParams;
        new Ajax.Updater("group-topiclist-container", "habblet/newtopic.php", {
            method: "post", evalScripts: true, parameters: A, onComplete: function (D, C) {
    ...
    preview: function (C) {
        ...
        new Ajax.Updater("new-" + A + "-preview", habboReqPath + "habblet/previewpost.php?x=" + A, {
            method: "post", parameters: E, onComplete: function (G, F) {
                Discussions.hideEditElements();
    ...
    save: function (F) {
        ...
        var H = Discussions.baseParams + "&message=" + G;
        var E = "post";
        var C;
        if (F != null) {
            E = "topic";
            H += "&topicName=" + F;
            C = habboReqPath + "habblet/savetopic.php"
        } else {
            var A = $("edit-type").value;
            var D = $("current-page").value;
            if (A == "update") {
                C = habboReqPath + "updatepost.php";
                H += "&postId=" + $("post-id").value
            } else {
                C = habboReqPath + "habblet/savepost.php";
                D = -1
            }
            H += "&page=" + D
        }
        new Ajax.Request(C, {
            method: "post", parameters: H, onComplete: function (K, J) {
                ...
                if (J && J.spam == "true") {
                    K.responseText.evalScripts();
                    Element.hide("new-" + E + "-preview");
                    Discussions.showEditElements()
                } else {
                    if (J && J.forbidden == "true") {
                        $("discussionbox").innerHTML = K.responseText;
                        return
                    } else {
                        if (E == "post") {
                            $("group-postlist-container").innerHTML = K.responseText;
                            K.responseText.evalScripts()
                        } else {
                            if (E == "topic") {
                                var I = K.responseText;
                                if (I != null && I.startsWith("viewthread.php?")) {
                                    document.location = I
                                } else {
                                    $("group-topiclist-container").innerHTML = K.responseText
                                }
                            }
                        }
                    }
    ...
    deletePost: function (A) {
        ...
        D += "&page=" + C;
        new Ajax.Request(habboReqPath + "habblet/deletepost.php", {
            method: "post", parameters: D, onComplete: function (F, E) {
    ...
    deleteTopic: function (A) {
        ...
        new Ajax.Request(habboReqPath + "habblet/deletetopic.php", {
            method: "post", parameters: C, onComplete: function (E, D) {
    ...
    openTopicSettings: function () {
        ...
        Dialog.moveDialogToCenter(C);
        Overlay.show();
        new Ajax.Request(habboReqPath + "habblet/opentopicsettings.php", {
            method: "post", parameters: D, onComplete: function (F, E) {
    ...
    deleteTopicConfirmation: function (D) {
        ...
        Overlay.show();
        Dialog.setAsWaitDialog(C);
        new Ajax.Request(habboReqPath + "habblet/confirm_delete_topic.php", {
            method: "post", onComplete: function (F, E) {
                Dialog.setDialogBody(C, F.responseText);
    ...
    saveTopicSettings: function (I) {
        ...
        new Ajax.Updater("group-postlist-container", habboReqPath + "habblet/savetopicsettings.php", {
            method: "post",
            parameters: E,
    ...
    showTopicNameErrorBubble: function (C) {
        if (!$(C).hasChildNodes()) {
            var A = Builder.node("div", {className: "rounded-red"}, L10N.get("myhabbo.discussion.error.topic_name_empty"));
            $(C).appendChild(A);
...
var WidgetRegistry = {
    _widgetsById: [], _widgetsByType: [], add: function (A, D, C) {
...
var FriendsWidget = Class.create({
    options: {searchUrl: "myhabbo/avatarlist_friendsearchpaging.php", ownerParameter: "&_mypage.requested.account="},
    ...
    showAccountDetails: function (A) {
        ...
        this.showId = A;
        new Ajax.Request(habboReqPath + "myhabbo/avatarlist_avatarinfo.php", {
            method: "post", parameters: this._getInfoParameters(), onComplete: function (E, D) {
...
var MemberWidget = Class.create(FriendsWidget, {
    options: {searchUrl: "myhabbo/avatarlist_membersearchpaging.php", ownerParameter: "&_groupspage.requested.group="},
    leaveFromGroup: function ($super, A) {
        Event.stop(A);
        openGroupActionDialog("habblet/ajax_groups_confirm_leave.php", "habblet/ajax_leavegroup.php", this.showId, this.ownerId, this)
    },
    removeFromGroup: function ($super, A) {
        Event.stop(A);
        openGroupActionDialog("habblet/ajax_group_confirm_remove.php", "habblet/ajax_removegroup.php", this.showId, this.ownerId, this)
    },
    giveRights: function ($super, A) {
        Event.stop(A);
        openGroupActionDialog("habblet/ajax_groups_confirm_give_rights.php", "habblet/ajax_groups_give_rights.php", this.showId, this.ownerId, this)
    },
    revokeRights: function ($super, A) {
        Event.stop(A);
        openGroupActionDialog("habblet/ajax_groups_confirm_revoke_rights.php", "habblet/ajax_groups_revoke_rights.php", this.showId, this.ownerId, this)
    },
...
GuestbookWidget.prototype = {
    initialize: function (F, E, D) {
        ...
        if ($("guestbook-open-dialog")) {
            ...
            Event.observe("guestbook-form-preview", "click", function (G) {
                ...
                if ($F("guestbook-message").length > 0) {
                    ...
                    new Ajax.Updater($("guestbook-preview-tab"), habboReqPath + "myhabbo/guestbook_preview.php", {
                        method: "post",
                        ...
                        onComplete: function () {
                            Event.observe("guestbook-form-post", "click", function (H) {
                                Event.stop(H);
                                new Ajax.Updater($("guestbook-entry-container"), habboReqPath + "myhabbo/guestbook_add.php", {
                                    method: "post",
                                    parameters: Form.serialize($("guestbook-form")) + "&widgetId=" + encodeURIComponent(this.widgetId),
                                    evalScripts: true,
                                    insertion: "top",
        ...
        if (C.className == "gbentry-delete") {
            Event.stop(D);
            var A = C.id.substring("gbentry-delete-".length);
            this.removeEntry(A)
        }
        if (C.className == "gbentry-report") {
            Event.stop(D);
            var A = C.id.substring("gbentry-report-".length);
            ReportDialogManager.show("guestbook", A, C, {
                setWidth: false,
                setHeight: false,
                offsetTop: 0,
                offsetLeft: -120
        ...
        if (C.scrollTop > 0 && (C.scrollTop + C.clientHeight == C.scrollHeight)) {
            ...
            new Ajax.Updater($("guestbook-entry-container"), habboReqPath + "myhabbo/guestbook_list.php", {
                method: "post",
        ...
        new Ajax.Request(habboReqPath + "myhabbo/guestbook_remove.php", {
...
GroupsWidget.prototype = {
    initialize: function (A, C) {
        ...
        new Ajax.Request(habboReqPath + "habblet/ajax_groupinfo.php", {
            method: "post",
...
TagWidgetPartial.prototype = {
    initialize: function (A) {
        ...
        var C = Event.element(D);
        var A = C.className.substring(C.className.lastIndexOf("-") + 1);
        if (C.className.indexOf("tag-delete-link") >= 0) {
            ...
            if (this.parentWidget.tagType == "group") {
                ...
                if (this.parentWidget.tagType == "avatar") {
                    new Ajax.Updater("profile-tag-list", "habblet/tags_ajax.php?key=p_remove", {
        ...
        new Ajax.Request("habblet/tags_ajax.php?key=add", {
            parameters: "accountId=" + C + "&tagName=" + encodeURIComponent(A), onSuccess: function (E) {
                var D = E.responseText;
                if (D == "valid" && $("profile-tags-status-field")) {
                    new Ajax.Updater("profile-tag-list", "habblet/tags_ajax.php?key=p_list", {parameters: "tagMsgCode=" + encodeURIComponent("valid") + "&accountId=" + C})
                }
                this.errorMessage(D)
            }.bind(this)
        })
...
ProfileWidget.prototype = {
    tagType: "avatar", initialize: function (C, A) {
        this.options = Object.extend({messageText: "Add as friend?", headerText: "Are you sure?"}, arguments[2] || {});
        this.accountId = C;
        if (A) {
        ...
        new Ajax.Request("myhabbo/friend_add.php", {
            parameters: "accountId=" + encodeURIComponent(this.accountId), onSuccess: function () {
...
GroupInfoWidget.prototype = {
    tagType: "group", initialize: function (C, A) {
        this.groupId = C;
        if (A) {
            this.loggedInAccountId = A
        }
    ...
    initialize: function (D, A, C) {
        ...
        if (A != "" && A != this.gameId) {
            ...
            new Ajax.Request(habboReqPath + "/myhabbo/highscorelist/setGameId", {
                method: "post", parameters: {widgetId: this.widgetId, gameId: A}, onComplete: function (C) {
        ...
        new Ajax.Request(habboReqPath + A, {
            method: "post",
            parameters: {widgetId: this.widgetId, gameId: this.gameId, type: C, period: E, page: ((D != -1) ? D : 0)},
...
var GroupFavoriteSelector = {
    init: function (E, A, C, D) {
        ...
        Event.stop(A);
        openGroupActionDialog("habblet/ajax_groups_confirm_favorite.php", "habblet/ajax_groups_select_favorite.php", GroupFavoriteSelector.ownerId, GroupFavoriteSelector.groupId, GroupFavoriteSelector.widget, GroupFavoriteSelector.onCompleteCallback)
    }, deselectFavorite: function (A) {
        Event.stop(A);
        openGroupActionDialog("habblet/ajax_groups_confirm_deselect_favorite.php", "habblet/ajax_groups_deselect_favorite.php", GroupFavoriteSelector.ownerId, GroupFavoriteSelector.groupId, GroupFavoriteSelector.widget, GroupFavoriteSelector.onCompleteCallback)
    }
};
...
HallOfFameWidget.prototype = {
    initialize: function (A) {
        ...
        new Ajax.Updater(this.containerEl, habboReqPath + "/myhabbo/halloffame/ajax", {
            method: "post", parameters: {game: A, widgetId: this.widgetId}, onComplete: function () {
                this._setupGameLinks()
            }.bind(this)
        })
    }
...
BestExpsWidget.prototype = {
    initialize: function (A) {
        ...
        new Ajax.Updater(this.containerEl, habboReqPath + "/myhabbo/bestexps/ajax", {
            method: "post",
            parameters: {widgetId: this.widgetId, type: A, offset: C},
            onComplete: function (D) {
                this._setupTypeLinks();
                this._setupOffsetLinks()
            }.bind(this)
...
var BadgesWidget = Class.create({
    options: {searchUrl: "myhabbo/badgelist_badgepaging.php", ownerParameter: "&_mypage.requested.account="},
    initialize: function (A, C) {
        this.ownerId = A;
        this.widgetId = C;
        ...
        new Ajax.Updater(this.containerElement, habboReqPath + this.options.searchUrl, {
            ...
            onComplete: function (H) {
                if (this.listHeight) {
                    ...
                    $(G).setStyle({height: this.listHeight + "px"})
                }
            }.bind(this)
        })
    }
